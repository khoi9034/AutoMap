...
def layer_text(layer: dict[str, Any]) -> str:
    return " ".join(str(value or "").lower() for value in [layer.get("title"), layer.get("layer_key"), layer.get("layer_name"), layer.get("role"), layer.get("category")])
...
def plain_legend_label(layer: dict[str, Any]) -> str:
    labels = {"primary_polygon_highlight": "Commercial zoning", "context_polygon_muted": "Zoning context", "boundary_outline": "Concord boundary", "parcel_outline": "Parcels", "floodplain_overlay": "100-year floodplain", "road_context": "Road context", "major_road": "Major roads", "route_line": "Road-following draft route"}
    role = str(layer.get("map_role") or layer.get("cartography_role") or context_role(layer))
