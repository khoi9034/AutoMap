...
Address-focused prompts follow the same focused-preview rule. Live address lookup is scoped to Cabarrus County, NC. If
the address is unmatched or clearly out of scope, AutoMap sets `can_preview=false`, shows `Address not found in Cabarrus
County records` guidance, and does not show a broad county map as a successful address preview.
